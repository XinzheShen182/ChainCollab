...
const ChorJs = () => {
			...
			panelsToListener[p] = () => {
				panels.forEach((otherPanel) => {
					if (p === otherPanel && !p.classList.contains("active")) {
						...
						document
							.getElementById(p.dataset.togglePanel)
							.classList.remove("hidden");
					} else {
						otherPanel.classList.remove("active");
						document
							.getElementById(otherPanel.dataset.togglePanel)
	...
	const js_create_new_diagram_listener = async (
		e: MouseEvent,
	): Promise<void> => {
	...
	useEffect(() => {
		...
		return () => {
			...
			console.log(
				"event listeners[js-download-diagram, js-download-svg, js-open-file, file-input, js-new-diagram, js-validate, js-upload, js-upload-dmn] removed",
			);
		};
	}, []);
	...
	useEffect(() => {
		...
		return () => {
			const removeModeler = async () => {
				...
				panels.forEach((panel) =>
					panel.removeEventListener("click", panelListeners[panel]),
				);
				isModelerHandling.current = false;
			};
			removeModeler();
	...
	return (
		...
		>
			<div className="content">
				...
				<div id="panel-toggle">
					<div
						data-toggle-panel="properties-panel"
				...
				<div className="buttons djs-container">
					<button
						id="js-new-diagram"
						className="icon-doc-new"
						title="Create new empty diagram"
						type="button"
					/>
					<button
						id="js-open-file"
						className="icon-folder"
						title="Select BPMN XML file"
						type="button"
					/>
					...
					<button
						id="js-validate"
						className="icon-bug"
						title="Check diagram for problems"
					></button>
					<div className="divider"></div>
					<button
						id="js-upload"
						className="icon-file-upload"
						title="Upload BPMN file"
					></button>
					<button
						id="js-upload-dmn"
						className="icon-file-upload"
			...
			<MainPage xmlDataMap={dmnIdXmlMap} onSave={addToDmnMap} />
			<UploadDmnModal
				dmnData={dmnIdXmlMap}
