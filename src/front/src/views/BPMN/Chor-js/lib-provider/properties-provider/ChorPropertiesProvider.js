...
import choreographyTaskDefinition from "./TaskDefinition";
...
export default function ChorPropertiesProvider(injector, bpmnFactory) {
	...
	this.getTabs = function (element) {
		...
		const detailsGroup = generalTab[0].groups.filter(
			(g) => g.id === "details",
		)[0];
	...
	ChorPropertiesProvider.prototype.conditionalEvent = function (
		group,
		element,
	) {
		const getValue = function (conditionalEvent, node) {
			const conditionalEventDefinition =
				eventDefinitionHelper.getConditionalEventDefinition(conditionalEvent);
			return {
		...
		const setValue = function (conditionalEvent, values) {
			...
			return cmdHelper.updateBusinessObject(conditionalEvent, condition, {
...
ChorPropertiesProvider.$inject = ["injector", "bpmnFactory"];
